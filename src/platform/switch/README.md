...
This is an **experimental, incomplete** attempt to turn the
[Mario Party 4 decompilation](https://github.com/mariopartyrd/marioparty4) into a
**native** Nintendo Switch homebrew app (`.nro`), rather than an emulator.

Because this is a decomp, the *game logic* is real C code compiled directly for
the Switch's ARM CPU. Only the platform layer (graphics, input, file IO, OS
services) is reimplemented on top of libnx. The long-term goal was a clean port
plus quality-of-life extras (native GameCube controller support on Switch, and
dynamic button glyphs that adapt to whichever controller is in use).

**Status: boots, runs the real engine loop, but does not render game content yet
(black screen).** It is shared as a starting point for anyone who wants to take
it further. See [Current status](#current-status) and
[Known blockers](#known-blockers) below — the blockers are the important part.
...
- **Windows gotcha:** the devkitA64 toolchain reads the Windows `TMP` env var. If
  ...
  `Cannot create temporary file in C:\WINDOWS`. Build from a normal shell/Command
  Prompt/PowerShell with a writable `TMP`, e.g.
  `set TMP=%LOCALAPPDATA%\Temp` (cmd) before `make`.
...
- **Eden / Ryujinx (emulator):** these load `.nro` homebrew directly (no keys, no
  ...
  emulator's virtual SD card (for Eden this is configured under the Yuzu data
  folder, e.g. `...\yuzu\sdmc\files\...`). Then load `marioparty4_switch.nro`.
...
### Logs
Engine `OSReport` output is routed to:
1. the system debug log (`svcOutputDebugString`) — visible in Ryujinx and in
   Eden's log window, and
2. a file `mp4_log.txt` written to the SD card root.

Watch for `Total files found: N` (N==0 means the `files/` folder isn't where the
port is looking).
...
ARM device, and both were the reason work paused here.
...
the engine reads **directly out of a data file** comes out byte-reversed. Fixed so
far at a few spots (search the tree for `__builtin_bswap`):
- `src/game/data.c` `GetFileInfo` (file header fields),
- `src/REL/bootDll/main.c` `NintendoDataDecode` (embedded logo header).

It recurs in every data format's loader (sprite anim banks, HSF models, message
data, etc.). The decode/decompression routines in `src/game/decode.c` are already
endian-safe (they read byte-by-byte).

### 2. 32-bit vs 64-bit pointer layout (the current wall)
This is the bigger one. GameCube data files store pointers/offsets as **4 bytes**.
The Switch is 64-bit, so pointers in memory are **8 bytes**. Any struct the engine
maps directly over file data that *contains pointer fields* therefore has a
**mismatched layout** — fields after the first pointer land at the wrong offset,
producing garbage and reads from near-null addresses.

Concrete example (where the sprite/logo rendering currently dies): `ANIMDATA` in
`include/game/animdata.h` is 20 bytes in the file (32-bit pointers) but 32 bytes
when compiled for 64-bit. `HuSprAnimRead` in `src/game/sprman.c` reads
`anim->bank`/`anim->pat`/`anim->bmp` at the wrong offsets, its "already relocated?"
check misfires, and relocation is skipped.
...
- **Per-format loaders** (most robust): for each on-disk format, read the 32-bit
  ...
  Do this once per format (sprites, HSF, etc.).
